...
public class MidiFile {
    ...
    private short num_tracks;
    private short quarter_note;
    private String trackID="";
    private int trackLength=0;

    
    private ArrayList<ArrayList<MidiEvent>> allevents; /** The raw MidiEvents, one list per track */
    private ArrayList<MidiTrack> tracks = new ArrayList<>(); ;  /** The tracks of the midi file that have notes */
    ...
    public MidiFile(byte[] data, String fileName) {
     ...
     * - The number, start time, and duration of each note.
     */
    
    private void parse(byte[] data){
        MidiFileReader midiFileReader = new MidiFileReader(data);
        headerID = midiFileReader.ReadAscii(4);
        headerLength = midiFileReader.ReadInt();
        trackMode = (short)midiFileReader.ReadShort();
        num_tracks = (short)midiFileReader.ReadShort();
        quarter_note = (short)midiFileReader.ReadShort();
        
        allevents = new ArrayList<ArrayList<MidiEvent>>();
        for (int tracknum = 0; tracknum < num_tracks; tracknum++) {
            allevents.add(ReadTrack(midiFileReader));
        ...
        timesig = new TimeSignature(numer, denom, quarter_note, (int)tempo);
    }
    ...
    private ArrayList<MidiEvent> ReadTrack(MidiFileReader file) {
        ...
        while (file.GetOffset() < trackend) {
            ...
            if (eventflag >= EventNoteOn && eventflag < EventNoteOn + 16) {
                ...
                throw new MidiException("Unknown event " + mevent.EventFlag,
                                             file.GetOffset()-1); 
            }
        }
    ...
    public void Write(FileOutputStream destfile, MidiOptions options) 
      throws IOException {
        ...
        WriteEvents(destfile, newevents, trackMode, quarter_note);
    }
        ...
        for (MidiTrack track : tracks) {
            ...
            for (MidiNote note : track.getNotes()) {
                if (note.getStartTime() < prevtime) {
                    throw new MidiException("Internal parsing error", 0);
                }
     ...
     * 4 seconds, and return the start times.
     */
    ...
    public String toString() {
        String result = "Midi File tracks=" + tracks.size() + " quarter=" + quarter_note + "\n";
